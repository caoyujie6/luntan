...
public class LoginSeriverImpl implements LoginSeriver {
   ...
   //根据板块名字查找版块id
    ...
    public int fatieren(int ren) throws SQLException {
        return impl.fatieren(ren);
    }

    @Override
    public Bankuai select(int id) {
        Bankuai bankuai=new Bankuai();
        try {
            bankuai=impl.selectBKname(id);
        } catch (SQLException e) {
            e.printStackTrace();
        }
        return bankuai;
    ...
    @Override
    public int selectHfTznum(int id) {
        int a=0;
        try {
            a=impl.selectHuTznum(id);
        } catch (SQLException e) {
            e.printStackTrace();
        }
        return a;
    ...
    public int pinglun(int ping) throws SQLException {
        return impl.pinglun(ping);
    }

    @Override
    public int list2(int yid) throws SQLException {
        return impl.list2(yid);
    }
